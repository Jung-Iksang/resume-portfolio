      ...
      <group>
        <group position={getPoint}>
          ...
          <group position={[0, -0.5, 0]}>
            ...
            <Text {...textProps} fontSize={0.2} position={[0, -0.6 - diff, 0]}>
              {point.subtitle}
            </Text>
            {point.details && point.details.length > 0 && (
              <>
                <mesh position={[
                  textAlign === 'right' ? -2 : 2,
                  -1.0 - (point.details.length - 1) * 0.25 / 2 - diff,
                  -0.01
                ]}>
                  <planeGeometry args={[4.5, point.details.length * 0.25 + 0.2]} />
                  <meshBasicMaterial color="#000000" transparent opacity={0.35} />
                </mesh>
                {point.details.map((detail, idx) => (
                  <Text key={idx} {...koDetailProps} position={[0, -1.0 - idx * 0.25 - diff, 0]}>
                    {detail}
                  </Text>
                ))}
              </>
